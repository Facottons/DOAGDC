...
DO_React_enrich <- function(p_cutoff = 0.05,
    ...
    if (grepl("crosstable", tolower(Tool))) {
        ...
        DIR <- paste0(PATH, "/Ontology_Results_", tolower(groupGen), "_", Tool,
                                                            "_", toupper(Name))
    }
    ...
    if (tolower(dataBase) == "gdc") {
        geneIDList_DE <- clusterProfiler::bitr(DEGenes_all$ensembl,
                                            fromType = "ENSEMBL",
                                            toType = "ENTREZID",
                                            OrgDb = "org.Hs.eg.db")$ENTREZID
        geneIDList_Universe <- clusterProfiler::bitr(TCGAExpression$ensembl,
                                                    fromType = "ENSEMBL",
                                                    toType = "ENTREZID",
                                            OrgDb = "org.Hs.eg.db")$ENTREZID
    } else {
    ...
    if (nrow(DO_Enriched_fdr) != 0){
        ...
        if (tolower(image_format) == "png") {
            png(file = paste0(DIR,
                            "/DO_Output/DO_EnrichPlot_10first_", pairName,
                            ".png"),
