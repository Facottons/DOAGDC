...
download_gdc <- function(dataType = "gene",
    ...
    size.par <- function(tumor, typeOfData, DB){
        ...
        size <- as.numeric(subset(x = jason,
                                    subset = data_category == typeOfData,
                                    "file_count"))
        return(size)
    }
    ...
    if (tolower(dataBase) == "legacy"){
        ...
        folder.name <- paste(tolower(dataBase), tolower(dataType),
                            "data", sep = "_")
    } else {
    ...
    tryCatch(status.gdc <- read.csv(status),
                error = function(e){
                stop(message(cat("GDC server or your internet conection is",
                                " off. \n Please try again later!")))
            })

    dir.create(path = file.path(workDir, "DOAGDC"), showWarnings = FALSE)
    ...
    if(tolower(dataBase) == "legacy"){
        ...
        if ("gene" %in% tolower(dataType) || "isoform" %in% tolower(dataType)){
            ...
            if ("gene" %in% tolower(dataType)) {
                ...
                dir.create(path = file.path(workDir, "DOAGDC",
            ...
            manifest.df[, c("center", "acl", "state_comment", "cases",
                            "tags")] <- NULL
        ...
        if ("gene" %in% tolower(dataType)) {
            ...
            size.par <- function(tumor){
                url <- paste0("https://api.gdc.cancer.gov/projects/TCGA-",
                                toupper(tumor),
            ...
            if (tolower(HTSeq) == "counts") {
                ...
                url <- paste0(url.inicio, "?pretty=true&expand=cases.samples.",
                            ...
                            "value%22:%5B%22HTSeq%20-%20FPKM-UQ%22,%22",
                            "HTSeq%20-%20Counts%22,%22HTSeq%20",
                            "-%20FPKM%22%5D%7D%7D,%7B%22op%22:%22in%22,",
    ...
    if ("mutation" %in% tolower(dataType)) {
        ...
        if (tolower(dataBase) == "legacy") {
            ...
            platform <- paste0("%22value%22:%5B%22Illumina%20GA%22,",
        ...
        json <- tryCatch(jsonlite::fromJSON(url, simplifyDataFrame = TRUE),
                error = function(e) stop(e),
                                ...
                                " does not have ", tolower(dataType),
                                "data!"))
        ...
        patient.code <- matrix(data = as.character(unlist(pre.cases)),
                            nrow = 10,
    ...
    if ("methylation" %in% tolower(dataType)) {
        ...
        platform <- paste0("%22value%22:%5B%22Illumina%20Human%20Methylation",
                "%20450%22,%22Illumina%20Human%20Methylation%2027%22%5D%7D%7D")

        if (tolower(Platform) %in% "all") {
            tmp <- paste0("Illumina Human Methylation 450|Illumina ",
    ...
    if ("protein" == tolower(dataType)){
        ...
        size <- size.par(tumor = tumor, typeOfData = "Protein expression",
                        DB = "legacy")
    ...
    if ("mirna" %in% strsplit(tolower(dataType), split = " ")[[1]][1] || "isoform expression quantification" %in% tolower(dataType)) {
        ...
        if (tolower(Platform) %in% "all") {
            ...
            platform <- paste0(",%7B%22op%22:%22in%22,%22content%22:%7B%22",
            ...
            platform <- paste0(",%7B%22op%22:%22in%22,%22content%22:",
        ...
        if (tolower(dataBase) == "legacy") {
            size <- size.par(typeOfData = "Gene expression", tumor = tumor,
                            DB = tolower(dataBase))
            ...
            size <- size.par(typeOfData = "Transcriptome Profiling",
                            tumor = tumor, DB = tolower(dataBase))
    ...
    if ("exon" == strsplit(tolower(dataType), split = " ")[[1]][1]) {
        ...
        size <- size.par(tumor = tumor, typeOfData = "Gene expression",
                        DB = "legacy")
