...
download_gdc <- function(dataType = "gene",
    ...
    dir.create(path = file.path(workDir, "DOAGDC", toupper(tumor)), showWarnings = FALSE)
    ...
    if(tolower(dataBase) == "legacy"){
        ...
        if ("gene" %in% tolower(dataType) || "isoform" %in% tolower(dataType)){
            ...
            if ("gene" %in% tolower(dataType)) {
                ...
                dir.create(path = file.path(workDir, "DOAGDC", toupper(tumor), "gene_data"),
                           showWarnings = FALSE)
                ...
                dir.create(path = file.path(workDir, "DOAGDC", toupper(tumor), "isoform_data"),
                           showWarnings = FALSE)
        ...
        if ("gene" %in% tolower(dataType)){
            ...
            dir.create(path = file.path(workDir, "DOAGDC", toupper(tumor), "gdc_gene_data"),
                       showWarnings = FALSE)
    ...
    if ("mutation" %in% tolower(dataType)){
        ...
        dir.create(path = file.path(workDir, "DOAGDC", toupper(tumor), folder.name),
                   showWarnings = FALSE)
