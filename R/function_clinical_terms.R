...
#' @param Name
#' @param workDir
#' @param tumor A character vector indicating which tumors should be used in the
...
clinical_terms <- function(Name,
    ...
    for_couple_tumors <- function(only_one_tumor){
        ...
        for (tumors in names(lista)){
            ...
            clinical_df <- data.table::fread(input = file.path(DIR, patient_file), sep = "\t")
        ...
        if (is.null(term_keyword)) {
            ...
            term_in_tumors <- apply(string_vars[["envir_link"]]$clinical_df, 2, table)
            ...
            terms_unique <- table(terms_repeted)[order(table(terms_repeted), decreasing = TRUE)]
            # all_possible_terms <- unique(names(terms_unique)[order(names(terms_unique))])

            tmp <- names(terms_unique)[grep(pattern = term_keyword, x = names(terms_unique),
            ...
            if (!only_one_tumor) {
                ...
                write.table(terms_with_keyword, file.path(workDir,
                            ...
                            row.names = FALSE,
                            col.names = "Term(s)")
                assign("terms_with_keyword", terms_with_keyword, envir = get(envir_link))
    ...
    if ("all" %in% tolower(tumor)) {
        ...
        if (!(paste(toupper(tumor), toupper(dataBase), gsub(" ", "_", dataType),
                    "tumor_data", sep = "_") %in% ls(all.names = TRUE, envir = .GlobalEnv))){
            assign(paste(toupper(tumor), toupper(dataBase), gsub(" ", "_", dataType), "tumor_data", sep = "_"),
                   new.env(parent=emptyenv()), envir = .GlobalEnv)
            envir_link <- paste(toupper(tumor), toupper(dataBase),
                                gsub(" ", "_", dataType), "tumor_data", sep = "_")
        } else{
            if (missing(env)) {
                envir_link <- paste(toupper(tumor), toupper(dataBase), gsub(" ", "_", dataType),
                                    "tumor_data", sep = "_")
            } else {
        ...
        dir.create(paste0(PATH, "/Clinical_Results.", tolower(string_vars[["envir_link"]]$dataType), "_",
                          toupper(string_vars[["envir_link"]]$Name)), showWarnings = FALSE)
        DIR <- paste0(PATH, "/Clinical_Results.", tolower(string_vars[["envir_link"]]$dataType), "_",
        ...
        if (missing("term")){
            ...
            term_in_tumors <- string_vars[["envir_link"]]$clinical_df[, term, drop = FALSE]
            message("Removing 'unusable subcategories...")
            term_in_tumors[, 1] <- gsub("\\[Not Available\\]", NA, term_in_tumors[, 1])
            term_in_tumors[, 1] <- gsub("\\[Unknown\\]", NA, term_in_tumors[, 1])
            term_in_tumors[, 1] <- gsub("\\[Not Evaluated\\]", NA, term_in_tumors[, 1])
            ...
            # list_patients_per_category <- vector("list", category_number)
            # names(list_patients_per_category) <- term_category
            ...
            selected <- eval(parse(text=paste0('string_vars[["envir_link"]]$', paste0(tmp))))
            # selected <- string_vars[["envir_link"]]$gene_tumor_selected_HIF3A
            ...
            # or
            # unname(sapply(rownames(framesList[[tumor_i]]), function(w){
            ...
            plot <- ggplot2::ggplot(final_df_plot,
                            ggplot2::aes(x=category, y=log2(expression + 1), fill = category)) +
                ggplot2::stat_boxplot(geom ='errorbar',  position=ggplot2::position_dodge(1)) +
                ggplot2::geom_boxplot(position=ggplot2::position_dodge(1),
                                      inherit.aes = TRUE, outlier.colour = NA) +
                ggplot2::geom_jitter(position=ggplot2::position_jitter(0.2), alpha = 0.5, color = "black") +
                ggplot2::labs(x = "Categories", y = "log2(RSEM+1)", title = term) +
                ...
                # ggplot2::scale_color_brewer(palette="Drdbu")
                # ggplot2::scale_color_manual(values=wesanderson::wes_palette(n=3, name="Darjeeling"))
            print(plot)
            dev.off()
            ...
            sink(file.path(DIR, paste0(term, "_ANOVA_Analysis_of_Variance_report.txt")))
            cat("Used category")
            cat("\n")
            ...
            if (ANOVA[["Pr(>F)"]][1] <= p_cutoff){
                ...
                #                         upr     p adj
                # Stage_II-Stage_I    1.9904699 0.3966100
                # Stage_III-Stage_I   1.7266644 0.0107990*
                # Stage_IV-Stage_I    0.7845169 0.9624011
                # Stage_III-Stage_II  1.4631864 0.9725363
                ...
                for (index in seq(1, length(groups_export))) {
                    ...
                    clinical_groups[[index]] <- final_df[selector, 2, drop = FALSE]
                    clinical_groups[[index]][, 1] <- droplevels(clinical_groups[[index]][, 1])
                }

                message("The selected groups after statistical analysis are:\n",
                        paste(names(clinical_groups), "\n"),
                        "\n\nPlease insert these names to use them in posterior analysis.")

                assign("clinical_groups_clinical", clinical_groups, envir = get(envir_link))
