...
#' @param networkType A character string indicating which network type should
#'    be used. WGCNA allows: "unsigned", "signed", and "signed hybrid". The
#'    default is "unsigned".
#' @param minModuleSize Numerical value specifying the minimum cluster size.
#'    The default is 15.
...
co_expression <- function(Data = NULL,
    ...
    if (is.null(loadCheckpoint)) {
        ...
        message("Your plot was saved in ",
                ...
                "cutHeight value.")
        cutHeight <- as.numeric(readline(prompt = "Insert the cutHeight value: "))
        ...
        if (tolower(cutHeight) != "none" && is.numeric(cutHeight)) {
            ...
            clust <- WGCNA::cutreeStatic(sampleTree, cutHeight = cutHeight,
                                        minSize = 10)
        ...
        if (!is.null(traitData)) {
            ...
            traitData[, 1:ncol(traitData)] <- sapply(colnames(traitData),
                                                    function(col){
                as.numeric(gsub(pattern = "\\[Not Available\\]",
                                replacement = "NA", x = traitData[, col],
                                perl = TRUE))
            })

            traitData[, 1:ncol(traitData)] <- sapply(colnames(traitData),
                                                    function(col){
                as.numeric(gsub(pattern = "\\[Not Applicable\\]",
                                replacement = "NA", x = traitData[, col],
                                perl = TRUE))
            })
            ...
            if (tolower(networkType) == "signed") {
                traitColors <- WGCNA::numbers2colors(datTraits, signed = TRUE,
                                                    naColor = "grey")
            } else {
        ...
        # scale-free topology fit index reaches 0.90. nGenes <- ncol(datExpr)
        nSamples <- nrow(datExpr)

        softPower <- as.numeric(readline(prompt = paste0("Insert the soft ",
                                                        "threshold value: ")))
    ...
    merge <- WGCNA::mergeCloseModules(datExpr, dynamicColors,
    ...
    if (tolower(image_format) == "png") {
        png(filename = file.path(DIR, paste0("genedendro_",
                                            MEDissThres, ".png")),
            width = Width, height = Height, res = Res, units = Unit)
    } else if (tolower(image_format) == "svg") {
        svg(filename = file.path(DIR, paste0("genedendro_",
                                            MEDissThres, ".svg")),
            width = Width, height = Height, onefile = TRUE)
    } else {
        stop(message("Please, Insert a valid image_format! ('png' or 'svg')"))
    ...
    if (!is.null(traitData)) {
        ...
        bwnet <- WGCNA::blockwiseModules(datExpr, maxBlockSize = 5000,
                                power = softPower, TOMType = networkType,
                                minModuleSize = minModuleSize,
                                reassignThreshold = 0,
                                mergeCutHeight = MEDissThres,
                                numericLabels = TRUE,
                                saveTOMs = FALSE, #or TRUE
                                saveTOMFileBase = file.path(DIR,
                                                            "TOM-blockwise"),
                                verbose = 3)
        ...
        if (tolower(image_format) == "png") {
            png(filename = file.path(DIR,
                                    "Gene_dendrogram_and_module_colors.png"),
                width = Width, height = Height, res = Res, units = Unit)
        } else if (tolower(image_format) == "svg") {
            svg(filename = file.path(DIR,
        ...
        WGCNA::plotDendroAndColors(geneTree,
                            ...
                            main = paste0("Single block gene dendrogram and",
                                            " module colors"),
                            dendroLabels = FALSE, hang = 0.03,
                            addGuide = TRUE, guideHang = 0.05)
        dev.off()
        ...
        singleBlockMEs <- WGCNA::moduleEigengenes(datExpr,
                                                    moduleColors)$eigengenes
        blockwiseMEs <- WGCNA::moduleEigengenes(datExpr,
                                                bwModuleColors)$eigengenes
    ...
    write.table(all_module, file = file.path(DIR, 'gene_modules.tsv'),
                col.names = c("gene_id", "module"),
    ...
    if (tolower(dataBase) == "gdc") {
        module <- all_module$mergedColors[grep(toupper(Name),
                                            all_module$probes)]
    } else if (tolower(dataBase) == "legacy") {
        if (suppressWarnings(is.na(as.numeric(Name)))) {
            # Symbol before |
            module <- all_module$mergedColors[grep(paste0(toupper(Name),
                                                        "\\|"),
                                                all_module$probes)]
        } else {
            # Entrez
            module <- all_module$mergedColors[grep(paste0("\\|",
                                                        toupper(Name)),
                                                all_module$probes)]
        }
    }
    ...
    pearson_values <- sapply(as.numeric(fileEdges$weight), function(x){
        unique(round(as.numeric(names(threshold)[threshold == round(x, 3)]),
                    3))[1]
    })
