...
groups_identification_mclust <- function(dataType,
    ...
    PLOT <- function (mc, data, nbBreaks, traceDensity = TRUE, title = "",
        ...
        d <- mclust::cdens(modelName = mc$modelName, data = p,
        ...
        legend("topright", legend = paste0("G", seq(1, mc[["G"]])),
            title = "Groups",
    ...
    if (!reRunPlots){
        ...
        assign("MCLUST.RESULT.FINAL", MCLUST.RESULT.FINAL,
        ...
        GROUPS <- as.data.frame(matrix(ncol = 2,
                            nrow = length(MCLUST.RESULT.FINAL$classification)))
        ...
        plot(Uncertainty.range, remaining.patients,
        ...
        axis(side = 2, seq(min(remaining.patients),
                                            (max(remaining.patients) + 30), 60),
        ...
        #example cutoff = 0.05
        abline(v = 0.05, lwd = 2)
        text(x = 0.05 + 0.01, y = 10, labels = "0.05",
        ...
        to.plot <- data.frame(xaxis = Uncertainty.range[2:length(Uncertainty.range)],
                            yaxix = sapply(2:length(remaining.patients),
                                            function(n){
                                                remaining.patients[n] - remaining.patients[n - 1]}))
        ...
        if (tolower(image_format) == "png") {
            png(filename = paste0(DIR,
                            "/mixture_log2_expression_", uncertaintyCutoff,
                            "_filtered_%01d.png"),
                width = Width, height = Height, res = Res, units = Unit)
        } else if (tolower(image_format) == "svg") {
        ...
        par(mar = c(5, 6, 4, 3.5))
        PLOT(mc = MCLUST.RESULT.FINAL,
            data = log2(FILTERED_RESULTS.filtered + 1),
            nbBreaks = nBreaks, las = 1,
            traceDensity = TRUE,
            cex.main = 1.4,
            cex.lab = 1,
        ...
        assign("MCLUST.RESULT.FINAL.filtered", MCLUST.RESULT.FINAL,
                                                    envir = get(envir_link))
    }
