...
dea_edgeR <- function(Name,
    ...
    #local functions ####
    plotBCV.modified <- function(y, xlab = "Average logCPM",
    ...
    exact_groups_fix <- function(tested, Pairs){
        ...
        edgeR::plotSmear(tested_local, de.tags = detags, las = 1,
                                                            ylab = "log2FC")
        abline(h = c(-log2(FC_cutoff), log2(FC_cutoff)), col = "blue", cex = 2)
        legend("topright", border = FALSE, bty = "n",
            legend=c("p.adj > FDR_cutoff", "p.adj < FDR_cutoff",
                                                            "log2FC_cutoff"),
    ...
    glmlrt_groups_fix <- function(tested, Pairs){
        ...
        if (Pairs > 0) {
            ...
            Results_Completed[[1]] <- Results_Completed_local
            resultadosDE[[1]] <- tableDE
            assign("Results_Completed_edgeR", Results_Completed,
    ...
    volcano <- function(Results_Completed, Pairs){
        ...
        Results_Completed_local$Colour[Results_Completed_local$log2FC >= log2(FC_cutoff) & Results_Completed_local$FDR <= FDR_cutoff] <- rgb(222, 22, 22, 50)
        Results_Completed_local$Colour[Results_Completed_local$log2FC < -log2(FC_cutoff) & Results_Completed_local$FDR <= FDR_cutoff] <- rgb(56, 50, 237, 50)
    ...
    dir.create(paste0(PATH, "/edgeR_Results.",
                    ...
                    showWarnings = FALSE)
    DIR <- paste0(PATH, "/edgeR_Results.", tolower(groupGen), "_",
                                                                toupper(Name))
    ...
    if (tolower(Method) == "exacttest") {
        ...
        if (group2_number > 2) {
            ...
            message("There are more than two group ",
        ...
        aGlmFit <- edgeR::glmFit(aDGEList, design,
        ...
        group2_number <- max(as.numeric(levels(Grupos_edgeR[, 1])))
        if (group2_number > 2) {
            message("There are more than two group combinations, ",
    ...
    write.csv(x = NormalizedExpression_TMM,
                        file = file.path(DIR, "TMM_Normalized_Expression.csv"))
    assign("NormalizedExpression.edgeR", NormalizedExpression_TMM,
                                                    envir = get(envir_link))
